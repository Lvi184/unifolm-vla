#!/bin/bash
# Simple training script without DeepSpeed for AGIBOT 2026 Competition Dataset
# Only trains action_model, freezes qwen_vl_interface
...
run_id=unifolm_vla_agibot_v1

output_dir=${run_root_dir}/${run_id}
mkdir -p ${output_dir}
cp $0 ${output_dir}/

# Run without DeepSpeed - just use python directly
cd /root/gpufree-data/unifolm-vla
source .venv/bin/activate

python src/unifolm_vla/training/train_unifolm_vla.py \
