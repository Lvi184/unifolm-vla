...
from unifolm_vla.datasets.acot_adapter import state159_to_21
...
def unnormalize_action(normalized_actions: np.ndarray, action_norm_stats: dict):
    ...
    # Handle shape mismatch: if model outputs 21D but norm stats are 40D
    if len(normalized_actions.shape) >= 1 and normalized_actions.shape[-1] == 21 and len(action_high.shape) >= 1 and action_high.shape[0] == 40:
        # Use first 21D of norm stats
        action_high = action_high[:21]
        action_low = action_low[:21]
        if mask is not None:
            mask = mask[:21]
...
def normalize_proprio(proprio: np.ndarray, norm_stats: dict):
    ...
    # Handle shape mismatch: normalize first 21D only (our projection dimension)
    # Check if proprio is 21D but norm stats are 159D
    if len(proprio.shape) >= 1 and proprio.shape[-1] == 21 and len(proprio_high.shape) >= 1 and proprio_high.shape[0] == 159:
        # Use first 21D of norm stats
        proprio_high = proprio_high[:21]
        proprio_low = proprio_low[:21]
        if mask is not None:
            mask = mask[:21]
...
logger.info("=" * 60)
logger.info(" UnifoLM-VLA Policy Server (ACoT direct training version) ")
logger.info("=" * 60)
...
metadata = {
    "model_name": "unifolm_vla_acot_direct",
...
async def handler(websocket: ws_server.ServerConnection):
    ...
    while True:
        try:
            ...
            # =======================================
            # Process proprioception (pad to 159D then project to 21D)
            # =======================================
            proprios = []
            for observation in observations:
                ...
                # Pad to 159D if shorter, then project to 21D - works for both original 159D and genie_sim 32D
                state_padded = np.zeros(159, dtype=np.float32)
                state_padded[:len(state)] = state
                state = state159_to_21(state_padded)
            ...
            if len(action) >= 32:
                ...
                action = action[:32]
                logger.info(f"Action was {len(action)}D, sliced to 32D for genie_sim")
